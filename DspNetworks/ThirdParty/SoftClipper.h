...
#include "src/SoftClipper.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct SoftClipperMetaData {
		SN_NODE_ID("SoftClipper");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using SoftClipper = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _SoftClipper, SoftClipperMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
